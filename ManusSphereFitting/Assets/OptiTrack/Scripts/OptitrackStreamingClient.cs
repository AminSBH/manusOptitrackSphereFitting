...
public static class OptitrackHiResTimer
{
    public struct Timestamp
    {
       ...
       /* public float SecondsSinceFloat(ulong refT)
        {
            Int64 deltaTicks = Convert.ToUInt32(m_ticks) - Convert.ToUInt32(refT);
...
public class OptitrackStreamingClient : MonoBehaviour
{
    ...
    //static void parseFrameData(.FrameOfMocapData data, NatNetML.NatNetClientML client)
    //{
    //}
    ...
    private void OnNatNetFrameReceived( object sender, NatNetClient.NativeFrameReceivedEventArgs eventArgs)
    {
        ...
        //output timeStamp when camera registered
        try
        {
            ...
            //NativeMethods.NatNet_Client_SecondsSinceHostTimestamp(pFrame, transmitTimestamp, out pTimeElapsed);
           // NatNetError retval = NaturalPoint.NatNetLib.NativeMethods.NatNet_Client_Create(out m_clientHandle);
            //NatNetException.ThrowIfNotOK(retval, "NatNet_Client_Create failed.");
            //NaturalPoint.NatNetLib.NativeMethods.NatNet_Client_SecondsSinceHostTimestamp(m_clientHandle, transmitTimestamp, out pTimeElapsed);
            /*Debug.Log(pTimeElapsed);
            Debug.Log(transmitTimestamp);
            Debug.Log(OptitrackHiResTimer.Now().m_ticks);*/

            //motionCap = eventArgs.MarshaledFrame;
            //ulong cameraTimeStamp = motionCap.CameraMidExposureTimestamp;

            //Debug.Log(OptitrackHiResTimer.Now().SecondsSince(transmitTimestamp));
            //Debug.Log("now" + OptitrackHiResTimer.Now().m_ticks.ToString());
            /*BitConverter.GetBytes()
            Marshal.Copy(motionCap.CameraMidExposureTimestamp, copiedBytes, 0, sizeof(long));*/

            // Debug.Log(motionCap.CameraMidExposureTimestamp);
